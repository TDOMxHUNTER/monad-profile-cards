...
import ProfileEditNFT from "./components/ProfileEditNFT";
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const [showProfileNFT, setShowProfileNFT] = React.useState(false);
  const [profileNFTData, setProfileNFTData] = React.useState(null);
  ...
  const handleSaveChanges = () => {
    ...
    // Trigger NFT minting for profile changes
    setProfileNFTData(formData);
    setShowProfileNFT(true);
    setShowSettings(false);
  };

  const handleNFTMintComplete = (success: boolean) => {
    if (success) {
      console.log('Profile NFT minted successfully!');
    } else {
      console.log('Profile NFT minting failed');
    }
    // Keep the notification visible for a bit longer
    setTimeout(() => {
      setShowProfileNFT(false);
      setProfileNFTData(null);
    }, 2000);
  };
  ...
  const handleDownloadCard = async () => {
    ...
    try {
      // Hide the settings button temporarily
      const settingsBtn = document.querySelector('.pc-settings-btn') as HTMLElement;
      const originalDisplay = settingsBtn?.style.display;
      ...
      const canvas = await html2canvas(cardElement, {
        ...
        ignoreElements: (element) => {
          // Ignore settings button and any floating elements
          return element.classList.contains('pc-settings-btn');
        }
      });

      // Restore settings button
      if (settingsBtn) settingsBtn.style.display = originalDisplay || '';

      // Create download link
      const link = document.createElement('a');
      link.download = `${name || 'profile'}-card.png`;
    ...
    <div className="pc-container">
      ...
      >
        ...
        <div className="pc-action-buttons">
          <button
            className="pc-action-btn pc-download-btn"
          ...
          <button
            className="pc-action-btn pc-settings-btn"
      ...
      {/* Profile Edit NFT Component */}
      {showProfileNFT && profileNFTData && (
        <ProfileEditNFT
